...
data_stats(high_low_filtered_wide,
           init_names,
           signif_y_manual_calc = 0.1,
           custom_ylim = c(-2, 3),
           file_name_pref = "stats01v24070802_",
...
#### Norm - Path
n_p <- ms_data_cleaned %>%
  mutate(group=ifelse(group!='N', 'P', 'N')) %>%
  mutate(group=factor(group))


## Standard data
n_p_stdz <- cbind(n_p[, c(1, 2)],
                       as.data.frame(scale(n_p[, c(-1, -2)])))


n_p_filtered_wide <- stat_test_filter(n_p_stdz, 0.05) %>%
  select(-file_id)

repeated_lipids2 <- search_doubles(n_p_filtered_wide, lipids_meta_cm)  

n_p_filtered_wide <- n_p_filtered_wide %>%
  select(-l022, -l084)


data_stats(n_p_filtered_wide,
           init_names,
           signif_y_manual_calc = 0.1,
           custom_ylim = c(-2, 3),
           file_name_pref = "stats01v24070802_",
           file_name_temp = "n_p_stdz_box",
           file_name_suff = "r01v24070601_d24070301",
           x_label = "Параметр",
           y_label = "Уровень",
           fill_label = "Группа",
